    ...
    let res = (|| -> Res<(), { loc_var!(Games) }> {
        ...
        if cli.gaming.stagger {
            (|| -> Res<()> {
                ...
                _ = GetSystemCpuSetInformation(None, 0, &mut required_cpu_set_infos_size, Some(proc_hnd), None);
                ...
                let staggered_cpu_set_ids = cpu_set_infos.iter()
                ...
                info!("{}: staggered cpu set ids: {}", module_path!(), staggered_cpu_set_ids.as_display());
