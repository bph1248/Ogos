...
pub enum ErrVar {
    ...
    FastImageResizeCreateFilter(#[from] fast_image_resize::CreateFilterError),
    FastImageResizeMapping(#[from] fast_image_resize::MappingError),
    FastImageResizeResize(#[from] fast_image_resize::ResizeError),
    FromUtf16(#[from] string::FromUtf16Error),
    FromUtf8(#[from] string::FromUtf8Error),
    Image(#[from] image::error::ImageError),
    Io(#[from] io::Error),
    JpegDecoder(#[from] jpeg_decoder::Error),
    Notify(#[from] notify_debouncer_full::notify::Error),
    NotifyDebounced(Vec<notify_debouncer_full::notify::Error>),
    ...
    ZenWebpDecode(#[from] zenwebp::DecodeError),
    ...
    FailedConstructViewportCenterCommand,
...
impl Display for ErrVar {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            FastImageResizeCreateFilter(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            FastImageResizeMapping(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            FastImageResizeResize(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            ...
            JpegDecoder(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            Notify(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            NotifyDebounced(inner) => write!(f, "{}: {}", as_ref_str!(self), inner.as_display()),
            LogSetLogger(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            NvApi(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            ParseFloat(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            ...
            WgpuPoll(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            ...
            ZenWebpDecode(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            Zip(inner) => write!(f, "{}: {}", as_ref_str!(self), inner),
            ...
            FailedConstructViewportCenterCommand => write!(f, "failed to construct viewport center command"),
            ...
            InvalidImageFormat { archive_i: archive_index, name } => write!(f, "invalid image format: archive index: {}, name: {}", archive_index, name),
            ...
            InvalidPngColorSpace { color_space } => write!(f, "invalid png color space: {}", color_space.as_display()),
            ...
            MissingConfigOption { name } => write!(f, "missing config option: {}", name),
