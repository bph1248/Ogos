...
static NVAPI_GPU_SET_DITHER_CONTROL_FN: Lazy<NvAPI_GPU_SetDitherControl_fn> = Lazy::new(|| unsafe {
...
struct _NV_GPU_DITHER_CONTROL_V1__bindgen_ty_1 {
    bits: NvU32,
    mode: NvU32
}
...
struct _NV_GPU_DITHER_CONTROL_V1 {
    ...
    bits: NV_DITHER_BITS,
    mode: NV_DITHER_MODE,
    caps: _NV_GPU_DITHER_CONTROL_V1__bindgen_ty_1
}
...
type NvAPI_GPU_GetDitherControl_fn = unsafe extern "C" fn(
    output_id: NvU32,
...
struct DISPLAYCONFIG_ADVANCED_COLOR_MODE(i32);
impl DISPLAYCONFIG_ADVANCED_COLOR_MODE {
    const SDR: Self = Self(0);
    const WCG: Self = Self(1);
    const HDR: Self = Self(2);
}
...
#[repr(C)]
#[allow(nonstandard_style)]
struct DISPLAYCONFIG_SET_HDR_STATE_0_0 {
    _bitfield: u32
}
...
#[repr(C)]
#[allow(nonstandard_style)]
union DISPLAYCONFIG_SET_HDR_STATE_0 {
...
fn enable_screensaver() -> ResVar<()> { unsafe {
...
fn get_display_friendly_name(path: DISPLAYCONFIG_PATH_INFO) -> Res1<U16CString> { unsafe {
...
pub fn get_display_mode(path: DISPLAYCONFIG_PATH_INFO) -> Res1<DisplayMode> { unsafe {
...
pub fn get_first_display_path() -> Res1<DISPLAYCONFIG_PATH_INFO> { unsafe {
...
pub fn get_screen_extent() -> Res1<Extent2d> { unsafe {
    let mut path_count = 0;
    let mut mode_count = 0;
    GetDisplayConfigBufferSizes(QDC_ONLY_ACTIVE_PATHS, &mut path_count, &mut mode_count).ok()?;
...
fn set_color_bit_depth(display_id: NvU32, bit_depth: ColorBitDepth) -> Res1<Option<ColorBitDepth>> { unsafe {
    use nvapi_530::*;
...
fn set_dither_control(gpu_hnd: NvPhysicalGpuHandle, display_id: NvU32, state: DitherState, bit_depth: DitherBitDepth, mode: DitherMode) -> Res1<Option<NV_GPU_DITHER_CONTROL>> { unsafe {
...
pub fn set_display_mode(op: SetDisplayModeOp) -> Res<Option<DisplayMode>, { loc_var!(DisplayMode) }> {
...
pub fn set_screen_extent(extent: Extent2dU) -> Res1<Option<Extent2dU>> { unsafe {
    let mut path_count = 0;
    let mut mode_count = 0;
    GetDisplayConfigBufferSizes(QDC_ONLY_ACTIVE_PATHS, &mut path_count, &mut mode_count).ok()?;
