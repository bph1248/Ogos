...
pub enum ErrVar {
    ...
    FailedReadFile { inner: io::Error, path: String },
...
impl Display for ErrVar {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            FailedReadFile { inner, path } => write!(f, "failed to read file: {}: {}", path, inner),
            ...
            FailedWriteFile { inner, path } => write!(f, "failed to write file: {}: {}", path, inner),
